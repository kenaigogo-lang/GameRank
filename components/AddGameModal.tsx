...
import { X, Loader2, Save, Info, Upload, Image as ImageIcon, ChevronDown, Tags, Check, RotateCcw, Clipboard, Globe, Search, Sparkles } from 'lucide-react';
...
export const AddGameModal: React.FC<AddGameModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const handleSelectImage = async (url: string) => {
    setIsSearching(true);
    try {
        // We use corsproxy to fetch the image as a blob to avoid tainted canvas issues
        ...
        alert("Failed to download image. Please try another one.");
    } finally {
        setIsSearching(false);
    }
  };
          ...
          <div className="flex gap-2 sm:gap-3">
            ...
            <div className="flex-1 space-y-1 min-w-0">
              ...
              <input
                ...
                className="w-full h-11 bg-slate-950 border border-slate-700 rounded-lg px-2 sm:px-3 text-white focus:outline-none focus:border-blue-500 text-sm"
              />
            </div>
          </div>
